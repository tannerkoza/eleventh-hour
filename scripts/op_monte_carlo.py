...
NAVIGATOR = "op"
NSIMS = 25
JS = np.arange(30, 45, 5, dtype=float)
...
def monte_carlo():
    conf, meas_sim, leo_meas_sim, corr_sim = op.setup_simulation(
        ...
        for _ in tqdm(range(NSIMS), desc=f"simulating with {js} dB J/S"):
            match NAVIGATOR.casefold():
                case "op":
                    results = op.simulate(
                        conf=conf,
                        gps_meas_sim=meas_sim,
                        leo_meas_sim=leo_meas_sim,
                        corr_sim=corr_sim,
                        disable_progress=DISABLE_PROGRESS,
                    )
...
def process_sim_results(results: op.SimulationResults, mc_results: dict):
